...
diff --side-by-side /tmp/A /tmp/B | grep "[<|>]" |
    sed "s#\([^ ]*\) *[|][ \t]*\([^ ]*\)#< \1\n> \2#g" | sed 's#[ \t]*[><][ \t]*##g' |
    grep -vE '(\\\\|\!|\%|\(|\)|\*|\*\=|\+|\+\=|\,|\-|\-\=|\.|\/|\:|\:\:|\=|\=\=\=|\?|\[|\]|\{|\}|\$end|AND|B_AND|BREAK|B_XOR|CASE|CHALLENGE|COL|CONST|CONTINUE|CS|DEC|DO|DOTS_ARITH_SEQ|DOTS_FILL|DOTS_GEOM_SEQ|DOTS_RANGE|ELSE|EOF|EQ|error|EXPR|FE|FIXED|FOR|FUNCTION|GE|GLOBAL|GT|IDENTIFIER|IF|IN|INC|INCLUDE|INTEGER|IS|LE|LT|METADATA|NAMESPACE|NE|NUMBER|OR|POW|PUBLIC|REFERENCE|RETURN|SHL|SHR|STAGE|SWITCH|STRING|SUBPROOF|TEMPLATE_STRING|T_STRING|WHEN|WHILE|WITNESS|LAST|FIRST|FRAME|TRANSITION)' | grep -v "'"
echo -e "\e[0m"
