...
class Compiler {
    ...
    constructor(Fr) {
        ...
        this.publics = new Ids(Fr);
        this.prover = new Ids(Fr);
    ...
    async parseSource(fileName, isMain = false) {
        ...
        for (let i=0; i<sts.length; i++) {
            ...
            if (s.type == 'include') {
                console.log(`=== INCLUDE === ${s.file.value} ==============================`);
                await this.doInclude(s);
            }
        }
    }
    async parseSource2(fileName, isMain = false) {
    ...
    colDeclaration(s, type, nextId, reserveExpressions = false) {
        ...
        for (const col of s.items) {
            ...
            console.log(`COL_DECLARATION(${colname}) type:${type} nextId:${nextId}`);
...
//        let ref = this.references.get(polname);
    ...
    getFullName(e) {
        ...
        const sp = (((e.subproof ?? 'this') === 'this') ? this.subproof : e.subproof);
